...
class Server {
    constructor(config) {
        ...
        this.devicesGroup = new DevicesGroup(this.webSocketConnections,
            this.config['mqtt_broker_url'], this.config['mqtt_topic_base']);
    ...
    updateProducts() {
        ...
        let callback = (data) => {
            ...
            this.devicesGroup.updateDevice('products', products, 'refreshData');
        };
        ...
        if (wsMessage.type === 'utf8') {
            ...
            try {
                ...
                if (message.event === 'deviceStatusUpdate') {
                    ...
                    } else if (message.deviceType === 'cart') {

                        if (message.action === 'addProduct' || message.action === 'removeProduct') {
                            this.devicesGroup.updateDevice('cart', message.product, message.action);
                        }
                    }
                    ...
                    let palmar = this.devicesGroup.devices.clientPalmar;
                    palmar.cartTotalRequest(message);
                } else if (message.event === 'paymentRequest') {
                    let palmar = this.devicesGroup.devices.clientPalmar;
                    palmar.paymentRequest();
                }
    ...
    createHTTPServer() {
        let server = http.createServer((req, res) => {
            ...
            if (['/market', '/dashboard'].includes(reqPath)) {
                fs.readFile(`views/${reqPath}.html`, 'utf8', (err, data) => {
