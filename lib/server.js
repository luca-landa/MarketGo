...
const Palmar = require("./devices/palmar").Palmar;
const ClientPalmar = require('./devices/client_palmar').ClientPalmar;
...
class Server {
    ...
    createShelves(products) {
        ...
        return devicesIdxs.map((idx, index) => {
            return new Shelf(idx, defaultQuantity, products[index]);
        });
    }
                ...
                if (message.event === 'deviceStatusUpdate') {
                    ...
                    if(message.deviceType === 'shelves') {
                        ...
                        if(message.action === 'removeNotification') {
                            this.updateDevice(message.deviceType, message.notification, message.action);
                        }
                    }
                    ...
                    let callback = (productData) => {
                        ...
                        let notification = {
                            message: `Name: ${productData.name}` + `Price: ${productData.price}`
                        };

                        this.updateDevice('clientPalmar', notification, 'addData');
                    };
