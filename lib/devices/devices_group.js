...
class DevicesGroup {
    ...
    updateDevice(deviceType, data, updateType) {
        ...
        if (permittedDeviceTypes.includes(deviceType)) {
            ...
            if (deviceType === 'products') {
                devices = this.devices['products'];
            } else if (deviceType === 'clientPalmar') {
                devices = this.devices.clientPalmar.getDataClone();
            } else {
                devices = this.devices[deviceType].map((device) => device.getDataClone());
            }
    ...
    getDevicesJSON() {
        ...
        let result = {
            shelves: shelves,
            staffPalmars: this.devices.staffPalmars,
            clientPalmar: this.devices.clientPalmar,
    ...
    createShelves(products, mqttBrokerUrl, mqttTopicBase) {
        ...
        return devicesIdxs.map((idx, index) => {
            return new Shelf(idx, defaultQuantity, minQuantity, maxQuantity, products[index], mqttBrokerUrl, mqttTopicBase);
        });
    }
